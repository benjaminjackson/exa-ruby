...
module Exa
  module Services
    module Websets
      class ListImportsTest < Minitest::Test
        def setup
          @connection = Exa::Connection.build(api_key: "test_key")
        end

        def test_initialize_with_connection_and_params
          service = ListImports.new(@connection, limit: 10)

          assert_instance_of ListImports, service
        end

        def test_call_gets_imports_list_endpoint
          ...
          service = ListImports.new(@connection)
          service.call

          assert_requested :get, "https://api.exa.ai/websets/v0/imports"
        end

        def test_call_returns_import_collection_object
          stub_request(:get, "https://api.exa.ai/websets/v0/imports")
            .to_return(
              status: 200,
              body: {
        ...
        def test_call_sends_pagination_parameters
          ...
          service = ListImports.new(@connection, cursor: "next_page", limit: 20)
          service.call
