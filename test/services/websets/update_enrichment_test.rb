# frozen_string_literal: true

require "test_helper"

class WebsetsUpdateEnrichmentTest < Minitest::Test
  def setup
    @connection = Minitest::Mock.new
  end

  def test_updates_enrichment
    response_body = {
      "id" => "enrich_abc123",
      "object" => "webset_enrichment",
      "status" => "pending",
      "websetId" => "ws_test",
      "title" => "Updated Title",
      "description" => "Updated description",
      "format" => "text",
      "options" => [{ "label" => "New Option" }],
      "metadata" => { "updated" => true },
      "createdAt" => "2024-01-15T10:00:00Z",
      "updatedAt" => "2024-01-15T12:00:00Z"
    }

    response = Minitest::Mock.new
    response.expect :body, response_body

    @connection.expect :patch, response, [
      "/websets/v0/websets/ws_test/enrichments/enrich_abc123",
      {
        description: "Updated description",
        format: "text",
        options: [{ label: "New Option" }],
        metadata: { "updated" => true }
      }
    ]

    service = Exa::Services::Websets::UpdateEnrichment.new(
      @connection,
      webset_id: "ws_test",
      id: "enrich_abc123",
      description: "Updated description",
      format: "text",
      options: [{ label: "New Option" }],
      metadata: { "updated" => true }
    )
    result = service.call

    assert_instance_of Exa::Resources::WebsetEnrichment, result
    assert_equal "enrich_abc123", result.id
    assert_equal "Updated description", result.description
    assert_equal "text", result.format

    @connection.verify
  end
end
